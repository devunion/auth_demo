...
_messages.Messages.getLoginInfo(function (msg) {
    switch (msg.auth_flow.type) {
        case 'basic':
            _auth.Auth.basicAuth(msg);
            break;

        case 'continuous':
            _auth.Auth.continuousAuth(msg);
            break;
    }
});
...
exports.FormDetector = exports.Form = undefined;
...
var SUBMIT_FIELD_RULES = ["input[type='submit']", "button[type='submit']"];

var Form = exports.Form = function () {
    function Form(login, pass, submit, form) {
    ...
    _createClass(Form, [{
        ...
        value: function fill(data) {
            _robot.Robot.typeString(this._login[0], data.login);
            _robot.Robot.typeString(this._pass[0], data.password);
        }
    }, {
        key: 'submit',
        value: function submit() {
            if (this._submit) {
                _robot.Robot.focus(this._submit[0]);
                _robot.Robot.click(this._submit[0]);
            } else {
                this._form.submit();
            }
        }
...
var FormDetector = exports.FormDetector = function () {
    function FormDetector() {
        _classCallCheck(this, FormDetector);
    }

    _createClass(FormDetector, null, [{
        key: '_getVisibleForms',
        value: function _getVisibleForms() {
            return $("form").filter(":visible");
        }
    }, {
        key: '_getField',
        value: function _getField(rules, form) {
            for (var i = 0; i < rules.length; i++) {
                var result = $(rules[i], form);
        ...
        key: '_getVisibleInputs',
        value: function _getVisibleInputs(form) {
            return $("input:not([type=hidden])", form).filter(":visible");
        }
    }, {
        key: '_isLoginForm',
        value: function _isLoginForm(login, pass, form) {
            return pass && login && this._getVisibleInputs(form).length <= MAX_INPUTS_FOR_LOGIN;
        }
    }, {
        key: 'getLoginForm',
        value: function getLoginForm() {
            var forms = this._getVisibleForms();

            for (var i = 0; i < forms.length; i++) {
                var form = forms[i];

                var login = this._getField(LOGIN_FIELD_RULES, form);
                var pass = this._getField(PASS_FIELD_RULES, form);
                var submit = this._getField(SUBMIT_FIELD_RULES, form);
...
var _actionNames = __webpack_require__(0);
...
var Messages = exports.Messages = function () {
    ...
    _createClass(Messages, null, [{
        key: "saveSubmitInfo",
        value: function saveSubmitInfo(hash, callback) {
            chrome.extension.sendRequest({
                action: _actionNames.SAVE_SUBMIT_INFO,
                domain: (0, _utils.getDomain)(),
                hash: hash
            }, callback);
        }
    }, {
        key: "getLoginInfo",
        value: function getLoginInfo(callback) {
...
// In the case of more complex logic we will need to memoize this function.
...
var BasicAuth = function () {
    ...
    _createClass(BasicAuth, [{
        key: '_submitForm',
        value: function _submitForm(form, data) {
            form.fill(data);

            _messages.Messages.saveSubmitInfo(data.hash, function () {
                form.submit();
            });
        }
    }, {
        ...
        value: function auth(data) {
            var loginForm = _forms.FormDetector.getLoginForm();
        ...
        key: '_canSubmit',
        value: function _canSubmit(msg) {
            return msg.last_domain !== (0, _utils.getDomain)() || msg.last_hash !== msg.logins[0].hash;
        }
    }]);
...
var ContinuousAuth = function () {
    function ContinuousAuth() {
        _classCallCheck(this, ContinuousAuth);
    }

    _createClass(ContinuousAuth, [{
        key: '_getValue',
        value: function _getValue(val, logins) {
            switch (val) {
                case '${LOGIN}':
        ...
        key: '_waitFor',
        value: function _waitFor(selector, callback) {
            var _this = this;
            ...
            if (el.length > 0) {
                ...
                setTimeout(function () {
                    return _this._waitFor(selector, callback);
                }, 20);
            }
        }
    }, {
        key: '_handleStep',
        value: function _handleStep(steps, logins) {
            var _this2 = this;

            if (steps.length > 0) {
                var step = steps.shift();

                this._waitFor(step.next, function (next) {
                    var val = _this2._getValue(step.value, logins);

                    _robot.Robot.typeString($(step.selector)[0], val);

                    if (step.next_action === 'click') {
                        _robot.Robot.click(next[0]);

                        _this2._handleStep(steps, logins);
                    }
                });
            }
        }
...
var Auth = exports.Auth = function () {
    ...
    _createClass(Auth, null, [{
        key: 'basicAuth',
        value: function basicAuth(data) {
            ba.auth(data);
        }
    }, {
        key: 'continuousAuth',
        value: function continuousAuth(data) {
            ca.auth(data);
        }
    }]);
...
function simulate(element, eventName) {
    ...
    if (!eventType) throw new SyntaxError('Only HTMLEvents and MouseEvents interfaces are supported. Desired event: ' + eventName);
...
var Robot = exports.Robot = function () {
    ...
    _createClass(Robot, null, [{
        ...
        key: 'typeString',
        value: function typeString(el, str) {
            el.focus();
